...
def score_symbol(symbol: str, df: pd.DataFrame, closed_trades: List[Dict[str, Any]]) -> Signal:
    ...
    # RSI sweet spot for swing trades
    if 45 <= rsi14 <= 68:
        score += 14
...
def update_positions(portfolio: Dict[str, Any], signal_map: Dict[str, Signal]) -> None:
    ...
    for position in list(portfolio.get("positions", [])):
        ...
        if gain_pct <= -stop:
            sell_position(portfolio, position, price, "stop loss")
        elif gain_pct >= target:
            sell_position(portfolio, position, price, "take profit")
        elif sig.score < 55:
            sell_position(portfolio, position, price, "score breakdown")
...
def generate_dashboard(portfolio: Dict[str, Any], signals: List[Signal], signal_map: Dict[str, Signal]) -> None:
    ...
    latest = {
        ...
        "top_signals": signals_df.head(10).to_dict(orient="records"),
        "positions": positions_rows,
        "closed_trades": portfolio.get("closed_trades", [])[-20:],
    }
...
<head>
  <meta charset="utf-8">
  <title>LevelUp Swing Trader</title>
  ...
  <style>
    ...
    .card {{
      ...
      border-radius: 12px;
      padding: 18px;
      box-shadow: 0 0 18px rgba(244,196,48,.08);
    }}
    ...
    th, td {{ padding: 9px; border-bottom: 1px solid #2f3542; text-align: right; }}
    th {{ background: #222837; color: #f4c430; }}
...
<body>
<div class="wrap">
  <h1>LevelUp Swing Trader</h1>
  <p>Fake money. Real market data. Daily swing-trading discipline.</p>
  ...
  <div class="cards">
    ...
    <div class="card"><div class="label">Return</div><div class="value">{return_pct}%</div></div>
  </div>

  <h2>Open Positions</h2>
  {pos_table}

  <h2>Top Swing Signals</h2>
  {top_table}
  ...
  <p class="note">
    Educational paper-trading dashboard only. This does not place real trades and is not financial advice.
